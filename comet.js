...
const cometsData = JSON.parse(cometsJsonString);

// Function to retrieve comet information by name
function getCometInfo(name) {
  return cometsData.find(c => c.object_name === name);
}function calculateCometPosition(comet, julianDate) {
    // Extract orbital parameters from the comet object
    const q = parseFloat(comet.q_au_1); // Perihelion distance (AU)
    const e = parseFloat(comet.e); // Eccentricity
    const i = parseFloat(comet.i_deg) * (Math.PI / 180); // Inclination (radians)
    const w = parseFloat(comet.w_deg) * (Math.PI / 180); // Argument of periapsis (radians)
    const node = parseFloat(comet.node_deg) * (Math.PI / 180); // Longitude of ascending node (radians)
    const T = parseFloat(comet.tp_tdb); // Time of perihelion passage (Julian Date)

    // Step 1: Calculate Semi-Major Axis (AU)
    const a = q / (1 - e); // Semi-major axis

    // Step 2: Calculate Mean Anomaly (M)
    const daysInYear = 365.25;
    const pDays = parseFloat(comet.p_yr) * daysInYear; // Period in days
    const n = (2 * Math.PI) / pDays; // Mean motion
    const M = n * (julianDate - T); // Mean anomaly

    // Normalize M to the range [0, 2π]
    const M_normalized = M % (2 * Math.PI);
    const M_adjusted = M_normalized < 0 ? M_normalized + 2 * Math.PI : M_normalized;

    // Step 3: Solve Kepler's Equation for Eccentric Anomaly (E) using a refined Newton's method
    let E = M_adjusted; // Initial guess
    for (let i = 0; i < 100; i++) {
        const delta = (E - e * Math.sin(E) - M_adjusted) / (1 - e * Math.cos(E));
        E -= delta; // Update E using Newton's method
        if (Math.abs(delta) < 1e-12) break; // Convergence criterion for high accuracy
    }

    // Step 4: Calculate heliocentric coordinates in the orbital plane
    const xPrime = a * (Math.cos(E) - e); // x' in the orbital plane
    const yPrime = a * Math.sqrt(1 - e * e) * Math.sin(E); // y' in the orbital plane

    // Step 5: Rotate to the ecliptic plane
    const x = xPrime * (Math.cos(w) * Math.cos(node) - Math.sin(w) * Math.sin(node) * Math.cos(i)) -
                yPrime * (Math.sin(w) * Math.cos(node) + Math.cos(w) * Math.sin(node) * Math.cos(i));
    const y = xPrime * (Math.sin(w) * Math.cos(node) + Math.cos(w) * Math.sin(node) * Math.cos(i)) +
                yPrime * (Math.cos(w) * Math.cos(i) * Math.sin(node) - Math.sin(w) * Math.sin(i));
    const z = yPrime * Math.sin(i);

    // Return the x, y, z coordinates with high precision
    return { x: Number(x.toFixed(12)), y: Number(y.toFixed(12)), z: Number(z.toFixed(12)) };
}
